...
export const TreeNodeView: React.FC<Props> = ({ node, isRoot = false, depth = 0 }) => {
    // Состояние открытых поддеревьев
    ...
    // Определение выхода штук за один крафт (например, из названия "ТКАНЬ X15" -> 15, либо из параметров)
    const matchYield = node.recipeName.match(/x(\d+)/i);
    const baseYieldPerCraft = matchYield ? parseInt(matchYield[1], 10) : 1;
    
    // Расчет количества крафтов для текущей шапки узла (например, 255 / 15 = 17 крафтов)
    const nodeCraftsCount = Math.ceil(node.craftedAmount / (baseYieldPerCraft || 1));

    return (
        <div className={`my-2 font-mono border rounded-none text-xs transition-all ${
            isRoot 
                ? 'bg-[#121419] border-orange-600/80 shadow-lg shadow-orange-950/20' 
                : 'bg-[#0f1115] border-zinc-800'
        }`}>
            {/* 1. ИНДУСТРИАЛЬНАЯ ШАПКА РЕЦЕПТА */}
            <div className={`px-3 py-2 flex items-center justify-between border-b gap-2 ${
                ...
                <div className="flex items-center gap-2 min-w-0 flex-wrap">
                    ...
                    <span className={`font-bold uppercase tracking-wider truncate text-xs ${
                        isRoot ? 'text-orange-400' : 'text-zinc-200'
                    }`}>
                        {node.recipeName}
                    </span>

                    {/* Общее количество штучек */}
                    <span className="text-[10px] px-1.5 py-0.2 font-bold bg-zinc-900 text-zinc-300 border border-zinc-700">
                        x{node.craftedAmount} шт.
                    </span>

                    {/* ПЛАШКА: Кол-во крафтов для шапки узла (например: 17 крафтов) */}
            ...
            {node.inputs && node.inputs.length > 0 && (
                <div className="p-2 space-y-1.5 bg-[#0a0b0e]">
                    {node.inputs.map((input, idx) => {
                        ...
                        // Вычисление требуемых крафтов для ингредиента
                        ...
                        return (
                            <div key={idx} className="space-y-1.5">
                                ...
                                <div
                                    ...
                                    className={`flex items-center justify-between p-1.5 border transition-colors select-none ${
                                        isCraftable
                                            ? 'cursor-pointer bg-[#14171e] hover:bg-[#1a1e27] border-orange-500/30 hover:border-orange-500/60'
                                ...
                                >
                                    <div className="flex items-center gap-2 min-w-0">
                                        ...
                                        <span className={`uppercase font-bold text-[11px] truncate ${
                                            isCraftable ? 'text-zinc-100' : 'text-zinc-400'
                                        }`}>
                                            {input.item.name}
                                        </span>
                                    </div>

                                    {/* Количество, циклы крафта и статус */}
                                    <div className="flex items-center gap-2 flex-shrink-0">
                                        <span className={`font-bold text-[11px] ${
                                            isCraftable ? 'text-orange-400' : 'text-zinc-300'
                                        }`}>
                                            x{input.amount} шт.
                                        </span>

                                        {/* Вывод кол-ва требуемых крафтов в строке ингредиента */}
                                        {isCraftable && (
